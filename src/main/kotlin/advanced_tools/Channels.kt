 ...
 * - Semantic Blocking
 * ---- Upon reception if there are no items
 * ---- Upon sending if channel's buffer is full
 * - CLosing
 * ---- Sending more items is an error
 * ---- Receiving is an error, if channel is empty
 * - Cancelling
 * ---- Closes channel and discards elements
 * - Configuring
 * ---- Buffer capacity
...
object Channels {
    ...
    // channel are concurrent queues ---- useful for exchanging data between coroutines. See GO
    ...
    suspend fun pushStocks(channel: SendChannel<StockPrice>) {
        ...
        LOGGER.info("Pushed an element")
        // when done you can close the channel
    ...
    /*
        Customize a channel - optional capacity
     */
    ...
    /*
        onUndeliveredElement triggers if the channel has elements that are about to be discarded this happens:
        - when a channel gets cancelled with elements inside
        - the send() throws an error, e.g. if the channel is closed
        - the receive() throws an error, e.g. if someone cancels the coroutine calling receive()
     */
